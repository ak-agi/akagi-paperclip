...
- **New agents start with every work lane off, on every create path.** The seed is applied in `agentService.create()` — the one choke point every create path goes through, including join-request approval and built-in agent provisioning, both of which pass `runtimeConfig: {}`. It is adapter-independent: keying it off "this adapter declares model profiles today" was itself a hole, because an adapter that declared none seeded nothing and every agent created under it gained all three work lanes the moment that adapter later declared one. Each lane the adapter declares is additionally seeded `{ enabled: false }` on the agent create and hire routes. A create request that declares a lane itself is left as declared.
- **The lane map is never reduced by a write.** `PATCH /api/agents/:id` replaces `runtime_config` wholesale, and an absent entry reads as enabled, so omitting a lane used to delete its switch and turn the lane back on. The `modelProfiles` subtree is therefore merged, not replaced: a lane the request names wins, a lane it omits keeps its stored entry. Re-enabling a lane is an explicit `{ enabled: true }` write. `POST /api/agents/:id/config-revisions/:revisionId/rollback` restores `runtime_config` wholesale from a snapshot and follows the same rule, so restoring a revision captured before this section existed cannot re-open a lane.
- **Only a board operator changes a work lane.** On create and hire, an agent-authenticated caller that writes `runtimeConfig.modelProfiles.<work lane>` is rejected with `403 agent_work_model_profile_change_forbidden` — there is no stored state to diff against, so naming a lane is the mutation. On PATCH and on config-revision rollback the rule is a **diff against stored state**: the request is rejected when the effective post-write lane state differs from the stored one, in either direction, including by omission. A caller that faithfully echoes the stored lanes is not rejected, because it changes nothing. The same caller may already request a lane per issue, so letting it enable a lane or repoint one at a costlier model would hand it the switch that bounds it. The reserved `cheap` recovery lane keeps its existing path.
- **A config-revision rollback is a full config write.** It restores `runtime_config` and `tier` wholesale, so it clears the same two checks a PATCH clears: the work-lane diff above, and the profile consent gate (`AGENT_PROFILE_CHANGE_CONSENT_FIELDS`) when the snapshot would change a consent field. `assertCanUpdateAgent` alone is an unconditional self-allow for an agent editing its own config and is not sufficient.
- **The `agent_safe` company import forces work lanes off and drops `tier`.** On that route the caller writes the whole bundle and nothing in it is board-vetted, so filling only the *absent* lanes is not a control — the caller simply declares the lane it wants, enabled, on the model it picked. `board_full` imports still honour a bundle-declared lane and tier, because a board operator vetted them.
- **Existing agents are not backfilled.** An agent created before this rule keeps absent work-lane entries, which still read as enabled, and no write path backfills them: seeding on the update path would silently disable the lanes of every already-deployed agent the first time anyone saved an unrelated runtime setting, while the board form still showed those lanes as on. Backfilling the deployed fleet is a separate operator decision. Board → agent config is where an operator switches them off.
...
The agent profile consent gate (`AGENT_PROFILE_CHANGE_CONSENT_FIELDS`) is PATCH-only, because it keys on `agent:<id>:profile` and that id does not exist before the agent does. `tier` is the one consent field that is optional at creation and that a later change binds to a work lane, so an agent-authenticated caller may not set it on `POST /agents` or `POST /agent-hires`: the request is rejected with `403 agent_create_tier_forbidden`. The `agent_safe` company import is the third create path an agent principal can reach, and it drops a bundle-declared `tier` to `null` for the same reason. Creating the agent with no tier stays allowed — `null` is the pre-existing behaviour — and the tier is then set through the consent-gated PATCH path. A config-revision rollback that would restore a tier goes through the consent gate as well. A board actor is unaffected.
