...
- permissionMode (string, optional): Grok permission mode passed via \`--permission-mode\`. Unset by default: Grok >= 1.0 enforces \`dontAsk\` as deny-by-default and it overrides \`--always-approve\`, so unattended runs rely on \`--always-approve\` alone. Runs are headless (\`grok --single\`, no interactive approver), so an explicitly configured \`dontAsk\` is dropped rather than forwarded — otherwise Grok cancels tool execution after one turn and the agent does no work
